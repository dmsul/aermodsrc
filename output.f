...
C     Begin LOOP Over Output Types
     ...
     &                       * RatioARM + BACKANN(IGRP)
                     END DO
                     ANNVAL(1:NUMREC,IGRP,1) = 
     &               ANNVAL(1:NUMREC,IGRP,1)/SNUM
                  ELSE
                     ANNVAL(1:NUMREC,IGRP,1) = 
     &               ANNVAL(1:NUMREC,IGRP,1)/SNUM
                  END IF
...
C     WRITE Error Message for Problem Writing to Postprocessor File
 ...
 9007 FORMAT('* AERMET (',A6,'): MODELING OPTIONS USED:',T93,A8,
     &      /'* ',23(A9,1X))
...
C                 Write Header Information
     ...
     &                                               TITLE1(1:68),RUNDAT
                  WRITE(IPLUNT(IVAL,IGRP,IAVE),9007) C_METVER, RUNTIM, 
     &                                              (MODOPS(I),I=1,23)
                  IF (PM25AVE .OR. NO2AVE .OR. SO2AVE) THEN
...
C           First Output Type Only (i.e., ITYP = 1)
            IF (ITYP .EQ. 1) THEN
               DO K = 1, IHST
                  ...
                  IF (IGRP .GT. 999) THEN
C                    Number of Source Groups Exceeds Limit of Field,
C                    Write Warning Message and Reset to 999
                     WRITE(DUMMY,'(I2.2,2X,I6.6)') IAVE, 
     &                                             MIN(IGRP,999999)
...
C        Write Header Information
         ...
         WRITE(IMXDCUNT(IGRP1),9007) C_METVER, RUNTIM,(MODOPS(I),
     &                                                 I=1,23)
C
         IF (PM25AVE) THEN
