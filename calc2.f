...
C        For urban stable boundary layers, limit plume rise to 1.25*ZI - HSP
         IF (URBSTAB) THEN
            IF (HSP .LT. 1.25D0*ZI) THEN
               DHFAER = MIN( DHFAER, 1.25D0*ZI - HSP )
            ELSE
               DHFAER = 0.0D0
            END IF
         END IF
...
C     Set Initial Wake Switches Based on Building Dimensions
      IF (DSBH.LE.0.000001D0 .OR. DSBW.LE.0.000001D0) THEN
...
C --- Calculate time scale (s) for random dispersion, based on 24 hours
      IF( L_LowWind2 )THEN
C ---    Use BIGT=12 instead of 24 for LowWind2 option
...
C        Use UMEAN instead of UEF with LOWWIND2 option
     ...
     &         UMEAN*UMEAN*(1.0D0-DEXP(-(TTRAV/TRAN)))
      ...
      IF( L_LowWind2 )THEN
C ---    Apply upper bound on FRAN under LOWWIND2 option; with 
C        a default value of 0.95, unless modified by the user
C        under the CO LOW_WIND keyword.
