...
C --- Allocate OLMGROUP arrays if needed
      IF( NUMOLM .GE. 1 )THEN
...
C ---          Write data to OLM debugging file
               if (OLMDEBUG) then 
                  write(OLMDBG,9987) kurdat, irec, iolm, 
     &                               srcid(isrc), o3conc, 
     &                               hrval(1), no, no2,
     &                               percentno2
9987              format(1x,i8,i5,i4,2x,a12,1x,f10.3,1x,
     &                                       3(e12.5,1x),f9.4)
               end if
...
C                    Exit the OLMGROUP Loop
                     EXIT
                  END IF
               END DO ! OLMGROUP Loop
            END IF
         END DO SOURCE_LOOP

         IF (NUMOLM .GE. 1) THEN
            DO IOLM = 1, NUMOLM
C ---          Loop through OLMGROUPs
...
C     Variable Declarations
      ...
      IMPLICIT NONE
      CHARACTER MODNAM*12
      DOUBLE PRECISION :: NOXCONC, GRPALL, BCKGRD, BACKtemp
...
C                       End Averaging Period LOOP
     ...
     &                     SHVALS(IREC,IGRP,ISEAS,IHOUR,ITYP)
                     ...
                     END IF
                  END DO
C                 End First Source Group LOOP
               END IF
            END DO

          END DO SOURCE_LOOP

C ---     Apply ARM_1hr ratio to AVEVAL array for all source groups,
C         after subtracting background concentrations (BACKAVE)   
          DO IGRP = 1, NUMGRP
            IF (GRP_BACK(IGRP)) THEN
C ---          Source group includes BACKGROUND
               DO IAVE = 1, NUMAVE
                  AVEVAL(IREC,IGRP,IAVE,1) = 
     &       MAX((AVEVAL(IREC,IGRP,IAVE,1)-BACKAVE(IGRP)),0.0D0) 
     &                         * ARM_1hr + BACKAVE(IGRP)
               END DO

C ---          Write data to ARM debugging file
               if (ARMDEBUG) then 
                  write(ARMDBG,9987) kurdat, irec, 
     &                               igrp, grpid(igrp), 
     &                               AVEVAL(IREC,IGRP,1,1),
     &                               BACKAVE(IGRP),
     &                               ARM_1hr
9987              format(1x,i8,i5,i6,2x,a8,1x,3(e12.5,1x))
               end if

            ELSE
C ---          Source group does not include BACKGROUND
               DO IAVE = 1, NUMAVE
                  AVEVAL(IREC,IGRP,IAVE,1) = 
     &            AVEVAL(IREC,IGRP,IAVE,1) * ARM_1hr
               END DO
...
C ---       Assign HRVALS based on CHI array
            HRVALS(IHOUR,ISRC) = CHI(1,ISRC,ITYP)

C ---       Add to GRPALL; used to determine the
C           NOXCONC for use in ARM
            GRPALL = GRPALL + HRVALS(IHOUR,ISRC)

C ---       Assign HRVALS to EV_AVEVAL array
            EV_AVEVAL(ISRC) = EV_AVEVAL(ISRC) + 
     &                        HRVALS(IHOUR,ISRC)
...
C              Check for Source Belonging to Group
               IF (IGROUP(ISRC,IGRP) .EQ. 1) THEN
                  GRPVAL(IGRP,IHOUR) = GRPVAL(IGRP,IHOUR) +
     &                                 HRVALS(IHOUR,ISRC)
                  GRPAVE(IGRP) = GRPAVE(IGRP) + HRVALS(IHOUR,ISRC)
               END IF
            END DO

         END DO EV_SOURCE_LOOP

C ---    Apply BACKGRND concentrations to GRPAVE array as applicable
         IF (L_BACKGRND) THEN
            DO IGRP = 1, NUMGRP
               CALL EV_SUMBACK
            END DO
...
C        Apply ARM_1hr to AVEVAL array for all source groups
         DO IGRP = 1, NUMGRP
            IF (GRP_BACK(IGRP)) THEN
               DO IAVE = 1, NUMAVE
                  GRPAVE(IGRP) = MAX((GRPAVE(IGRP)-BACKAVE(IGRP)),0.0D0)
     &                                 * ARM_1hr + BACKAVE(IGRP)
                  GRPVAL(IGRP,IHOUR) = MAX((GRPVAL(IGRP,IHOUR)-
     &                                             BACKAVE(IGRP)),0.0D0)
     &                                 * ARM_1hr + BACKAVE(IGRP)
               END DO
            ELSE
               DO IAVE = 1, NUMAVE
                  GRPAVE(IGRP) = GRPAVE(IGRP) * ARM_1hr
                  GRPVAL(IGRP,IHOUR) = GRPVAL(IGRP,IHOUR) * ARM_1hr
               END DO
            END IF
         END DO
...
C     Variable Declarations
      ...
      IMPLICIT NONE
      CHARACTER MODNAM*12
      DOUBLE PRECISION :: NOXCONC, GRPALL, BCKGRD
...
C        Calculate ARM2 Ratio based on GRP ALL w/o BG
         IF (GRP_BACK(INDX_GRPALL)) THEN
            NOXCONC = AVEVAL(IREC,INDX_GRPALL,1,1) - 
     &                    BACKAVE(INDX_GRPALL)
         ELSE
            NOXCONC = AVEVAL(IREC,INDX_GRPALL,1,1) 
         END IF
...
C        Calculate ARM2 ratio, RatioARM2, based on NOXCONC
         RatioARM2 = (-1.1723D-17*NOXCONC**6) + 
     &                (4.2795D-14*NOXCONC**5) - 
     &                (5.8345D-11*NOXCONC**4) + 
     &                (3.4555D-08*NOXCONC**3) - 
     &                (5.6062D-06*NOXCONC**2) - 
     &                 2.7383D-03*NOXCONC + 1.2441D+00
...
C ---    Apply RatioARM2 to AVEVAL array for all source groups,
C        after subtracting background concentrations (BACKAVE)   
         DO IGRP = 1, NUMGRP
            IF (GRP_BACK(IGRP)) THEN
               DO IAVE = 1, NUMAVE
                  AVEVAL(IREC,IGRP,IAVE,1) = 
     &       MAX((AVEVAL(IREC,IGRP,IAVE,1)-BACKAVE(IGRP)),0.0D0) 
     &                       * RatioARM2 + BACKAVE(IGRP)
               END DO
               IF (PERIOD .OR. ANNUAL) THEN
                  ANNVAL(IREC,IGRP,1) = 
     &       MAX((ANNVAL(IREC,IGRP,1)-BACKANN(IGRP)),0.0D0)
     &                    * RatioARM2 + BACKANN(IGRP)
               END IF
               IF (ISEAHR(IGRP) .EQ. 1) THEN
                  SHVALS(IREC,IGRP,ISEAS,IHOUR,1) = 
     &            SHVALS(IREC,IGRP,ISEAS,IHOUR,1) * RatioARM2
               END IF
            ELSE
               DO IAVE = 1, NUMAVE
                  AVEVAL(IREC,IGRP,IAVE,1) = 
     &            AVEVAL(IREC,IGRP,IAVE,1) * RatioARM2
               END DO
               IF (PERIOD .OR. ANNUAL) THEN
                  ANNVAL(IREC,IGRP,1) = 
     &            ANNVAL(IREC,IGRP,1) * RatioARM2
               END IF
               IF (ISEAHR(IGRP) .EQ. 1) THEN
                  SHVALS(IREC,IGRP,ISEAS,IHOUR,1) = 
     &            SHVALS(IREC,IGRP,ISEAS,IHOUR,1) * RatioARM2
...
C ---          Write data to ARM2 debugging file
               if (ARM2DEBUG) then 
                  write(ARMDBG,9987,err=991) kurdat, irec, 
     &                               igrp, grpid(igrp), 
     &                               AVEVAL(IREC,IGRP,1,1),
     &                               BACKAVE(IGRP),
     &                               NOXCONC, RatioARM2
9987              format(1x,i8,i5,i6,2x,a8,1x,4(e12.5,1x))
               end if
         END DO

         go to 1234

991      write(*,*) 'Error writing to ARM2 Debug file', kurdat
...
C ---    Assign GRPALL concentration to NOXCONC
         NOXCONC = GRPALL 
...
C        Apply RatioARM2 to AVEVAL array for all source groups
     ...
     &                               * RatioARM2 + BACKAVE(IGRP)
            ...
            ELSE
               DO IAVE = 1, NUMAVE
                  GRPAVE(IGRP) = GRPAVE(IGRP) * RatioARM2
                  GRPVAL(IGRP,IHOUR) = GRPVAL(IGRP,IHOUR) * RatioARM2
               END DO
            END IF
         END DO
...
C        O3CONC expressed in ug/m3
         domO3moles= VOLdom * O3CONC/(1.0D6 * 48.0D0) ! O3CONC is ug/m3

c        get moles of ozone in the combined plume
         sumO3moles= VOLsum * O3CONC/(1.0D6 * 48.0D0) ! O3CONC is ug/m3
