...
C --- Check for L_NewMetData flag; if surface file header shows current
C     version date (i.e., 11059, L_OldMetVer=.F.), but the wind data 
C     source/adjustment flags are missing, issue warning message if 
C     L_NewMetData = .FALSE., but only issue warning once.
      IF( .NOT. L_OldMetVer .AND. .NOT. SCREEN .AND.
     &         IMETMSG.EQ.0 .AND. .NOT. L_NewMetData )THEN
C ---    Set L_OldMetVer = .T.
         L_OldMetVer = .TRUE.
...
C        Write warning message regarding short-term averages since first calendar day
C        is incomplete
         IF (IHOUR .GT. 1 .AND. (PM25AVE .OR. NO2AVE .OR. SO2AVE) .AND. 
     &                                                  L_MAXDCONT) THEN
...
C           Write Warning Message: Short-term averages for first calendar day may not be valid
            CALL ERRHDL(PATH,MODNAM,'W','488','FirstDay')
         END IF
...
C           of data file; adjust ISYEAR to match data file (IYR)
...
C           of data file (since DAYRANGE keyword is being used); 
C           adjust ISYEAR to match data file (IYR)
            WRITE(DUMMY,'(''StartYR '',I4)') IYR
...
C           start date (ISDATE) for Re-started model run
            WRITE(DUMMY,'(I8.8)') ISDATE - (ISDATE/100000000)*100000000
...
C        DATE:    May 12, 1999
C
C        MODIFIED:   To include call to GRDEPS, for calculation of
...
C           Set the stability logical variables
            ...
            IF (FULLDATE.GE.ISDATE .AND. IPROC(JDAY).EQ.1) THEN
C
...
C     Compute sigmaV from nonmissing wind speed and sigmaTHETA
     ...
     &    PFLSA(LEVEL) .LT. 99.0D0 )THEN
         ...
         EPSIL = DSIN(SIGRAD) * ( 1.0D0 - GSIGV * SIGRAD )
...
C        Compute Urban-Rural Temperature Difference (DELTRUR = 12.0)
...
C ---       Compute Urban Convective Mixing Height
            ZIURB(IURB) = 400.0D0 * (URBPOP(IURB)/REFPOP) ** 0.25D0
...
C----    Modified to use 4-digit year (IYR) instead of 2-digit year (IYEAR)
     ...
     &   WSTAR, OBULEN, SFCZ0, UAVG, SVAVG, SWAVG, UATZI, SVATZI,
     &   SWATZI, VPTGZI
         WRITE (DBMUNT, 6118)
         DO I = MXGLVL, 1, -1
...
C
 6115 FORMAT( 1X, 80('-'),//,'  SURFACE AND PROFILE MET DATA:',/,
     1  T48, 'MONIN-     SFC',/,T17,
     2  'MIXING   SFC                   OBUKHOV   ROUGH.',/,T17,
     3  'HEIGHT   TEMP    U*      W*    LENGTH    LENGTH',/,
     4  '  YR  MO DA HR',
     5  '    (M)    (K)   (M/S)   (M/S)    (M)       (M)',//,
     6  1X,I4,3I3,2X,F6.1,2X,F5.1,1X,F6.3,1X,F7.3,2X,F7.1,3X,F7.3///,
