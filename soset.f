...
C           Issue fatal error message; Group ALL not found
            IF (ARM) THEN
               CALL ERRHDL(PATH,MODNAM,'E','299','ARM Option')
            ELSE IF (ARM2) THEN
               CALL ERRHDL(PATH,MODNAM,'E','299','ARM2 Option')
            END IF
         END IF             
      END IF
...
C*---	Modified to handle LINE sources
      IF (FIELD(4).EQ.'LINE') THEN
	   IF (IFC .LE. 2) THEN
...
C*---		  Modified  to add handling of LINE source type
               IF (SRCTYP(ISRC).EQ.'LINE') THEN
...
C     Set Up The Source Group Array
      IF (GRPID(IGRP) .EQ. 'ALL' .AND. .NOT.CONT) THEN
         DO I = 1, NUMSRC
            IGROUP(I,IGRP) = 1
         END DO
C ---    Include BACKGRND concentrations in group ALL by default;
C        but allow for NOBACKGROUND in source group ALL
         IF (L_BACKGRND) THEN
            IF (FIELD(4) .EQ. 'NOBACKGROUND' .OR. 
     &          FIELD(4) .EQ. 'NOBACKGRND') THEN
C ---          Do not include BACKGRND concentrations in group ALL
               GRP_BACK(IGRP) = .FALSE.
