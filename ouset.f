...
C        WRITE Error Message:  Maximum Value Selected Exceeds NMAX
         WRITE(DUMMY,'(I8)') NMAX
         CALL ERRHDL(PATH,MODNAM,'E','275',DUMMY)
         GO TO 999
...
C     Write Header to File for Formatted Plot Files.  Skip Header if FATAL.
     ...
     &                                     RUNDAT
 9005    FORMAT('* AERMOD (',A6,'): ',A68,5X,A8)
         WRITE(IPSUNT(INDGRP,INDAVE),9007) C_METVER, RUNTIM, 
     &                                    (MODOPS(I),I=1,23)
 9007    FORMAT('* AERMET (',A6,'): MODELING OPTIONS USED:',T93,A8,
     &         /'* ',23(A9,1X))
...
C        WRITE Error Message:  Maximum Value Selected Exceeds NMAX
         ...
         CALL ERRHDL(PATH,MODNAM,'E','275',DUMMY)
         GO TO 999
      ELSE IF (INUM .GT. IMXVAL(INDAVE)) THEN
C        WRITE Error Message:  Maximum Value Selected Exceeds IMXVAL
...
C     Write Header to File.  Skip Header if FATAL.
 ...
 9005    FORMAT('* AERMOD (',A6,'): ',A68,5X,A8)
         WRITE(IMDUNT(INDGRP),9007) C_METVER, RUNTIM, (MODOPS(I),I=1,23)
 9007    FORMAT('* AERMET (',A6,'): MODELING OPTIONS USED:',T93,A8,
     &         /'* ',23(A9,1X))
...
C           Write Error Message: Invalid Numerical Field
            WRITE(DUMMY,'(I8)') NHIVAL
            CALL ERRHDL(PATH,MODNAM,'E','270',KEYWRD)
            GO TO 999
