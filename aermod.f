...
C                        Draft Version Dated 13350
C 
C                             December 16, 2013
C
C               *** SEE AERMOD MODEL CHANGE BULLETIN MCB#9 ***
C
...
C           OLM value (variable OLMVAL) being zeor (0) before to avoid
C           a zero-devide in calculating PercentNO2.
C
      ...
      IF (PVMRMDBG) THEN
         ...
         IF (.NOT.L_OPENED) THEN
            ...
            IF (.NOT.L_OPENED) THEN
               ...
               WRITE(PVMDBG,'(''PVMRM Debug File:  '',51x,a8,/70x,a8/)')
     &                                                rundat, runtim
               WRITE(PVMDBG,9001)
      ...
      IF (OLMDEBUG) THEN
         ...
         IF (.NOT.L_OPENED) THEN
            ...
            IF (.NOT.L_OPENED) THEN
               ...
               WRITE(OLMDBG,'(''OLM Debug File:    '',51x,a8,/70x,a8/)')
     &                                                rundat, runtim
               WRITE(OLMDBG,9002)
9002           FORMAT(3X,'DATE',3X,'IREC IOLM  SRCID',11X,
     &               'O3CONC',4X,'HRVAL',8X,'NO_VAL',6X,'NO2_VAL',
     &               3X,'PercentNO2')
            ELSE
C ---          Unit is already opened, issue error message
...
C --- Open file with ARM/ARM2 debugging output, but first check 
C     for potential file unit conflict
      IF (ARMDEBUG) THEN
         L_OPENED = .FALSE.
         ...
         IF (.NOT.L_OPENED) THEN
            ...
            IF (.NOT.L_OPENED) THEN
               ...
               WRITE(ARMDBG,'(''ARM Debug File:    '',51x,a8,/70x,a8/)')
     &                                                rundat, runtim
               WRITE(ARMDBG,9003)
9003           FORMAT(3X,'DATE',3X,'IREC  IGRP   GRPID',6X,'AVEVAL',6X,
     &                   'BACKGRND',6X,'ARMRatio')
            ELSE
C ---          Unit is already opened, issue error message
      ...
      IF (ARM2DEBUG) THEN
         ...
         IF (.NOT.L_OPENED) THEN
            ...
            IF (.NOT.L_OPENED) THEN
               ...
               WRITE(ARMDBG,'(''ARM2 Debug File:   '',51x,a8,/70x,a8/)')
     &                                                rundat, runtim
               WRITE(ARMDBG,9004)
9004           FORMAT(3X,'DATE',3X,'IREC  IGRP   GRPID',6X,'AVEVAL',6X,
     &                   'BACKGRND',6X,'NOXCONC',5X,'ARM2Ratio')
            ELSE
C ---          Unit is already opened, issue error message
...
C        Write the model options (MODOPS) to the output file
         WRITE ( DBGUNT, 200 ) (MODOPS(I), I=1,23)
  100    FORMAT ( ' Title: ', A68, / '        ', A68 / )
  200    FORMAT ( ' OPTIONS: ', / 23(1X,A9), / )
...
C ---             Use single "background" O3 value from OZONEVAL keyword
               ...
               ELSE
C ---             Set O3CONC to 0.0 for full conversion
                  O3CONC = 0.0D0
               END IF
            ELSE
...
C ---          Process Hourly Values for OLM Option
            ...
            ELSE IF (ARM .AND. .NOT.CLMHR  .AND. .NOT.MSGHR) THEN
C ---          Process Hourly Values for ARM Option
               CALL ARM_CALC

            ELSE IF (ARM2 .AND. .NOT.CLMHR  .AND. .NOT.MSGHR) THEN
...
C           Flush HRVAL Arrays (1:NUMTYP)
            ...
            IF (PVMRM .OR. OLM .OR. ARM2 .OR. ARM) THEN
...
C*    Variable Declarations
      ...
      INTEGER :: IO3YR, IO3MN, IO3DY, IO3HR, IO3YR2, I, J
      INTEGER :: FULLO3HR(6)
      LOGICAL :: L_ReadErr
...
C ---          Initialize ReadErr and EndErr for this sector
     ...
     &                                              O3CONC
               ...
               IF (O3CONC .GT. 0.0D0 .AND. O3CONC .LT. 900.0D0) THEN
...
C ---          Initialize ReadErr and EndErr for this sector
     ...
     &                                              O3TEMP
               ...
               IF (O3TEMP .GT. 0.0D0 .AND. O3TEMP .LT. 900.0D0) THEN
C ---             Valid hourly value; convert to ug/m3 if needed
                  IF (O3FILUNITS .EQ. 'PPB') THEN
...
C*    Variable Declarations
      ...
      INTEGER :: IBGYR, IBGMN, IBGDY, IBGHR, IBGYR2, I, J
      INTEGER :: FULLBGHR(6)
...
C        Adjust format statement based on page number
     ...
     &                                              MIN(IPAGE,99999999)
      ...
      END IF
      WRITE(IOUNT,9030) (MODOPS(I),I=1,11)
      WRITE(IOUNT,9040) (MODOPS(I),I=12,23)
...
90292 FORMAT(1X,'*** AERMET - VERSION ',A6,' ***',3X,'*** ',A68,' ***',
     &       8X,A8,/T116,'PAGE',I8)
 9030 FORMAT(1X,'**MODELOPTs: ',11(1X,A9))
...
C           Write a warning to the user - latitude out-of-range
            CALL ERRHDL( PATH, MODNAM, 'W', '381', ALAT(3:10) )
         END IF
...
C           Write a warning to the user - error decoding the latitude
            CALL ERRHDL( PATH, MODNAM, 'W', '382', ALAT(3:10) )
...
C     Initialize logical for BG data inputs
      L_BACKGRND = .FALSE.
C     Initialize temporary logicals for LowWind1 and LowWind2 BETA options
      L_LW1_tmp = .FALSE.
      L_LW2_tmp = .FALSE.
...
C                 "target" source group under MAXDCONT option
                  IF (.NOT. MAXDWARN .AND. 
     &                      SUMHNH(IR,JGRP,IVAL) .GT. 0.0D0) THEN
                     IF( (SUMVAL_MAXD(IVAL,JGRP,JGRP,IR) -
     &                    SUMHNH(IR,JGRP,IVAL))/
